...
- **Serving areas** (V009, `serving_area` + `_position` / `_team` / `_member`): ministries/societies/governing bodies. Named `serving_area` (not `church_service`) to avoid colliding with the service layer; UI says "Serviço".
  ...
  - **Membresia com equipes**: quando o serviço tem ao menos uma equipe cadastrada, vínculo `MEMBERSHIP` exige `teamId` (validado no service); supervisão e coordenação podem ficar no escopo geral (`team_id` nulo) ou de equipe.
