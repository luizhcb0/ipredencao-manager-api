...
- **Firebase behind an optional Lambda proxy**: `firebase.lambda.name` set → token checks go through `lambda/firebase-proxy` (keeps App Runner instances small/cheap); blank → in-process Admin SDK. Both paths must stay behaviorally identical.
...
- **Serving areas** (V009, `serving_area` + `_position` / `_team` / `_member`): ministries/societies/governing bodies. Named `serving_area` (not `church_service`) to avoid colliding with the service layer; UI says "Serviço".
  - **Soft-delete exception**: unlike the rest of the domain ("hard deletes only"), areas/cargos/equipes carry an `active` flag — deactivate to hide from new vínculos while preserving history; hard delete is allowed only when nothing references them (else the service throws 409 "desative em vez de excluir"). Membership history lives in `serving_area_member_history` (DB triggers on INSERT/UPDATE/DELETE; `deleted_at` set on removal).
  ...
  - **Membresia com equipes**: quando o serviço tem ao menos uma equipe **ativa**, vínculo `MEMBERSHIP` exige `teamId` (validado no service); coordenação pode ficar no escopo geral (`team_id` nulo) ou de equipe. **Supervisão é sempre do serviço inteiro**: um vínculo `SUPERVISION` com `teamId` é rejeitado.
  ...
  - **Cargo `kind` is immutable once the cargo has any vínculo** — else a MEMBERSHIP→SUPERVISION flip would retroactively promote everyone and bypass the supervisor rule. Name/sort_order/active stay editable.
